...
public class ShulkerRainMod implements ModInitializer {
			...
			dispatcher.register(CommandManager.literal("start")
					.executes(context -> {
						...
						if (!playerDataMap.containsKey(playerId)) {
							playerDataMap.put(playerId, new PlayerShulkerRainData(player));
						}

						PlayerShulkerRainData data = playerDataMap.get(playerId);
						if (!data.isActive()) {
							data.start();
							source.sendFeedback(() -> Text.literal("游戏开始！记得给B站@浓硫酸酸不酸一键三连哦~~"), false);
						} else {
							source.sendFeedback(() -> Text.literal("游戏已经开始了~~"), false);
						}
						return 1;
					}));
			...
			dispatcher.register(CommandManager.literal("stop")
					.executes(context -> {
						...
						if (playerDataMap.containsKey(playerId)) {
							...
							if (data.isActive()) {
								data.stop();
								source.sendFeedback(() -> Text.literal("游戏停止，感谢游玩！"), false);
							} else {
								source.sendFeedback(() -> Text.literal("Shulker rain is not active!"), false);
							}
						} else {
							source.sendFeedback(() -> Text.literal("No active shulker rain found!"), false);
						}
						return 1;
					}));
		});
	...
	static class PlayerShulkerRainData {
		...
		private void generateInitialShulkers() {
			...
			// 面积翻倍：从256x256格扩大至512x512格，数量减半：从64个减少至32个
			for (int i = 0; i < 32; i++) {
				int offsetX = random.nextInt(512) - 256; // 范围从-128到128翻倍为-256到256
				int offsetZ = random.nextInt(512) - 256; // 范围从-128到128翻倍为-256到256
		...
		private void generatePeriodicShulkers() {
			...
			// 面积翻倍：从128x128格扩大至256x256格，数量减半：从32个减少至16个
			for (int i = 0; i < 16; i++) {
				int offsetX = random.nextInt(256) - 128; // 范围从-64到64翻倍为-128到128
				int offsetZ = random.nextInt(256) - 128; // 范围从-64到64翻倍为-128到128
				int y = 100 + random.nextInt(101);
	...
	static class FallingShulker {
		...
		public boolean update() {
			if (isLanded || !shulkerEntity.isAlive() || shulkerEntity.isRemoved()) {
				return false;
			}
